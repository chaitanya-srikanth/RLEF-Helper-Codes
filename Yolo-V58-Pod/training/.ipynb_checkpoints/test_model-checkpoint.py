...
def nms(predicted_boxes, predicted_labels, confidence_scores, iou_threshold=0.5):
    ...
    if not detections:
        print("No detections initially")
        return list(predicted_boxes), list(predicted_labels), list(confidence_scores)
    ...
    while detections:
        ...
        detections = [detection for detection in detections if not calculate_iou(current_detection[0], detection[0]) > iou_threshold]
...
def main(test_csv_path, test_json_path, dataset_path, models_path, output_file, statistics_file, hyperparameters):
    ...
    for image_id in database:
        ...
        predicted_boxes, predicted_labels, confidence_scores = nms(predicted_boxes, predicted_labels, confidence_scores)
        ...
        for pred_box, pred_class, conf_score in zip(predicted_boxes, predicted_labels, confidence_scores):
            ...
            ious = [calculate_iou(pred_box, true_box) for true_box, true_class in zip(ground_truth_annotations, ground_truth_labels)]
            ...
            ious = [calculate_iou(pred_box, true_box) for true_box, true_class in zip(ground_truth_annotations, ground_truth_labels) if true_class == pred_class]
            max_iou = max(ious) if ious else 0
        ...
        for true_box, true_class in zip(ground_truth_annotations, ground_truth_labels):
            ...
            ious = [calculate_iou(true_box, pred_box) for pred_box, pred_class in zip(predicted_boxes, predicted_labels) if pred_class == true_class]
            max_iou = max(ious) if ious else 0
        ...
        print("resource_wise_ground_truths------>  ", resource_wise_ground_truths)
        print("resource_wise_detections------>  ", resource_wise_detections)
        print("resource_wise_aps.values------>  ",resource_wise_aps.values())
        print("resource_wise_aps------>  ",resource_wise_aps)
        print("resource_wise_mAP------>  ",resource_wise_mAP)
        ...
        try:
            output_df = output_df._append({'resource_id': filename_2_resource_id[database[image_id]["file_name"]],
                                           ...
                                           'groundTruthAnnotations':
                                               test_csv[test_csv['name'] == database[image_id]["file_name"]][
            ...
            output_df = output_df.append({'resource_id': filename_2_resource_id[database[image_id]["file_name"]],
                                           'label': '',
                                           'predicted': '',
                                           'groundTruthAnnotations':
                                               test_csv[test_csv['name'] == database[image_id]["file_name"]][
