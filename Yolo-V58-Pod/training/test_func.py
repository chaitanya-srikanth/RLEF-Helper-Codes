    ...
    for ann in jsfile["annotations"]:
        ...
        if project_type == 'detection':
            ...
            if single_cls:
                ...
                bbox = [0, (xmax + xmin) / (2 * 640), (ymax + ymin) / (2 * 480), (xmax - xmin) / 640,
                      (ymax - ymin) / 480]

        elif project_type == 'segmentation':
            bbox = [0] if single_cls else [ann["category_id"]]
            for i in range(len(poly) // 2):
                _ = poly[2 * i]/640.0
                bbox.append(_)
                _ = poly[2 * i+ 1]/480.0
...
class testing_function():
    ...
    def test(self):
        ...
        for val_index, checkpoint in enumerate(self.model_details["startCheckpointFileArray"]):
            ...
            single_cls = checkpoint['collectionHyperparameter']['single_cls']
            ...
            try:
                ...
                # )
                csv_to_json_convert(self.test_csv_path, test_json_path, self.test_dataset_path, single_cls)
                json_to_txt_convert(test_json_path,  self.model_details['_id'], self.test_parent_dir, Config.PROJECT_TYPE, single_cls)
